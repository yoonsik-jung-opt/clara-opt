    ...
    try:
        ...
        if state_i and state_i.is_optimal and state_i.basis_inverse is not None:
            try:
                ...
                delta_b = b * rng2.uniform(-0.2, 0.2, m)
                new_b = b + delta_b
        ...
        memory_mb = (m * m * 8) / (1024 * 1024)  # B^-1 size
...
def main():
    ...
    tasks = [{"n": n, "m": m, "seed": seed} for n, m in SIZES for seed in SEEDS]
    n_workers = args.workers

    print(f"Exp 6: Scalability ({len(tasks)} instances, workers={n_workers})...")
    ...
    with mp.Pool(n_workers) as pool:
        for i, r in enumerate(pool.imap_unordered(process_single, tasks)):
            if r is not None:
                results.append(r)
                print(f"  [{i+1}/{len(tasks)}] {r['instance']}: {r['internal_status']} ({r['time_internal']}s)")
            else:
                print(f"  [{i+1}/{len(tasks)}] failed")
